...
> **Owner (execution/tools):** Original dev — see [`PLATFORM_OWNER_GUIDE.md`](./PLATFORM_OWNER_GUIDE.md)  
> **Working copy:** `llmwork/AI-Pentesting-Tool/`  
> **Date:** July 2026  
> **Related:** [`PLATFORM_OWNER_GUIDE.md`](./PLATFORM_OWNER_GUIDE.md) · [`ARCHITECTURE_AND_WORKFLOW_EXPLAINED.md`](./ARCHITECTURE_AND_WORKFLOW_EXPLAINED.md) · [`HYBRID_PLATFORM_BLUEPRINT.md`](./HYBRID_PLATFORM_BLUEPRINT.md) · [`LLM_INTEGRATION.md`](./LLM_INTEGRATION.md)
...
| **Outputs** | `CommanderDecision`: which phase to run next, brief for phase agent, final report markdown |
...
**Respect the user's goal in natural language** — pass it in the handoff/situational brief; the LLM reads CURRENT SITUATION before choosing tools.
...
- Pass the **user goal** and evidence summary — do not invent findings
...
- `service_enumeration` entries for `--top-ports` when prior full scan timed out
...
6. **User goals in natural language** stay in the prompt; situational brief drives tool choice from evidence
...
- `HandoffPacket.to_prompt()` includes findings + situational context

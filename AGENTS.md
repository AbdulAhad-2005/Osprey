...
- _"I just found a reusable technique that will help on FUTURE targets (a bypass, a chain, an org playbook)"_ → **`platform_propose_skill`** — capture it as a new operator skill so you (and future runs) can reuse it. Use it ONLY for genuinely new, transferable methodology — not a restatement of an existing skill (you can already read several at once with `platform_skills`), and not per-target facts (those go in `platform_think` / the graph). Cite what it worked against in `evidence=`. It's inert until the operator approves it (`/skill approve`), so mention that you proposed one and keep working.

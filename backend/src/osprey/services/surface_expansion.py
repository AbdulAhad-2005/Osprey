...
# run_expansion_to_fixpoint (the recon-fixpoint-then-vuln-dispatch driving
# loop, and its include_vuln_dispatch boolean gate) moved to
# services/investigation_director.py's run_to_completion — plans/harness/09-
# dual-mode-planner.md Step 2/2b/5. That loop is now driven by
# DeterministicPlanner + priority.should_unlock_phase instead of a hardcoded
# pass count and a caller-supplied boolean; run_expansion_pass below (the
# actual capability the loop calls each iteration) is unchanged, reused
# as-is, not rewritten.

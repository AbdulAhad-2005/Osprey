"""Dynamic LLM-based structuring for tool output no deterministic parser recognizes.

Hand-written regex parsers cannot keep pace with the space of tool output
formats this platform runs against — new NSE scripts, new tools, unusual
flags all produce shapes nobody has written a parser for yet. Rather than
storing an unstructured 2000-char blob (the old, and still final, fallback)
for anything outside known formats, this asks the already-configured
platform LLM (``services/llm_service.py``) to extract typed Finding-shaped
JSON from the raw output. Output is strictly validated against the Finding
schema before it's ever returned; if the LLM is unavailable, unconfigured, or
returns something that doesn't validate, callers get an empty list and fall
back to the old raw-observation Finding — this never blocks or crashes the
parsing pipeline, it only upgrades structure when it genuinely can.
"""

from __future__ import annotations

import json
import logging
import re

from pydantic import ValidationError

from osprey.schemas.finding import (
    ClaimSeverity,
    EvidenceGrade,
    Finding,
    FindingConfidence,
    FindingType,
    clamp_claim_severity,
)
from osprey.services.llm_service import LLMServiceError, get_llm_service, llm_configured

logger = logging.getLogger(__name__)

_MAX_INPUT_CHARS = 12_000
_MAX_DYNAMIC_FINDINGS = 25

# A deterministic parser that recognizes the overall output shape but hits an
# unparseable sub-block (e.g. an NSE script neither the vulns.lua nor the
# vulners shape covers) tags its raw-capture placeholder Finding with this so
# the async layer (registry.parse_tool_output) can try structuring it via the
# LLM before falling back to keeping the raw block. Any parser can opt an
# individual chunk of text into this — not just the NSE parser.
NEEDS_DYNAMIC_STRUCTURING_TAG = "needs_dynamic_structuring"

_FINDING_TYPES = [t.value for t in FindingType]

_SYSTEM_PROMPT = (
    "You extract structured security findings from raw penetration-test tool "
    "output. You are a strict extractor, not an analyst: report only what the "
    "text actually states, never infer, guess, or add anything not directly "
    "present in the text. If the text has nothing finding-worthy, return an "
    "empty list. Output ONLY a JSON array, no prose, no markdown fences."
)

_INSTRUCTIONS = (
    "Each array item must be an object with these keys:\n"
    '  "finding_type": one of ' + json.dumps(_FINDING_TYPES) + "\n"
    '  "title": short (<160 char) label for the fact\n'
    '  "description": one sentence, plain language\n'
    '  "evidence": the exact snippet of the input text that supports this (<500 char)\n'
    '  "evidence_grade": "observed" if the text directly shows this fact (e.g. a live '
    'response, a completed scan result), "inferred" if it is a derived/indirect '
    'signal, "unverified" if it is speculative or you are not confident\n'
    '  "severity": one of "none","info","low","medium","high","critical" — how '
    'serious this fact is IF it is a vulnerability/misconfiguration, else "none"\n'
    "Return [] if nothing is extractable. Do not fabricate CVEs, hosts, or ports "
    "not present in the text."
)

_JSON_ARRAY_RE = re.compile(r"\[.*\]", re.DOTALL)
_FENCE_RE = re.compile(r"^```(?:json)?\s*|\s*```$", re.MULTILINE)


async def structure_unparsed_output(
    tool_name: str,
    stdout: str,
    *,
    engagement_id: str = "",
    run_id: str = "",
    target: str = "",
    phase: str = "",
) -> list[Finding]:
    """Best-effort: turn raw stdout into structured Findings via the platform
    LLM. Returns [] (never raises) on any failure so callers always have the
    raw-observation fallback to lean on.

    Checked up front, not just left to fail into the except below: an
    MCP-driven session (OpenCode, Claude Desktop, any external harness) has
    its OWN calling LLM already doing this exact job — the CLI's own system
    prompt spells it out explicitly ("you are the fallback, not a second
    hidden model") — so the backend's copy of LLM_MODEL is frequently just
    never configured for these setups, on purpose. Calling complete()
    anyway used to mean a full max_retries+1 round of real network attempts
    against an unusable config on every single unparsed tool output, purely
    to land on the exact same [] this check returns immediately.
    """
    text = (stdout or "").strip()
    if not text or not llm_configured():
        return []

    truncated = text[:_MAX_INPUT_CHARS]
    note = (
        ""
        if len(text) <= _MAX_INPUT_CHARS
        else f"\n[...truncated, {len(text) - _MAX_INPUT_CHARS} more chars omitted]"
    )

    try:
        llm = get_llm_service()
        response = await llm.complete(
            messages=[
                {"role": "system", "content": _SYSTEM_PROMPT},
                {
                    "role": "user",
                    "content": (
                        f"Tool: {tool_name}\nTarget: {target or 'unknown'}\n\n"
                        f"{_INSTRUCTIONS}\n\n--- RAW OUTPUT ---\n{truncated}{note}"
                    ),
                },
            ],
            temperature=0.0,
            max_tokens=3000,
        )
    except LLMServiceError as exc:
        logger.debug("Dynamic parsing fallback unavailable for %s: %s", tool_name, exc)
        return []
    except Exception:
        logger.exception("Dynamic parsing fallback errored for %s", tool_name)
        return []

    try:
        content = response["choices"][0]["message"]["content"] or ""
    except (KeyError, IndexError, TypeError):
        return []

    items = _extract_json_array(content)
    if not items:
        return []

    findings: list[Finding] = []
    for item in items[:_MAX_DYNAMIC_FINDINGS]:
        finding = _to_finding(
            item,
            tool_name=tool_name,
            engagement_id=engagement_id,
            run_id=run_id,
            target=target,
            phase=phase,
        )
        if finding is not None:
            findings.append(finding)
    return findings


def _extract_json_array(content: str) -> list | None:
    text = (content or "").strip()
    if text.startswith("```"):
        text = _FENCE_RE.sub("", text).strip()
    try:
        data = json.loads(text)
    except (json.JSONDecodeError, ValueError):
        match = _JSON_ARRAY_RE.search(text)
        if not match:
            return None
        try:
            data = json.loads(match.group(0))
        except (json.JSONDecodeError, ValueError):
            return None
    return data if isinstance(data, list) else None


def _to_finding(
    item: object,
    *,
    tool_name: str,
    engagement_id: str,
    run_id: str,
    target: str,
    phase: str,
) -> Finding | None:
    if not isinstance(item, dict):
        return None

    try:
        ftype = FindingType(str(item.get("finding_type") or "observation"))
    except ValueError:
        ftype = FindingType.OBSERVATION
    try:
        grade = EvidenceGrade(str(item.get("evidence_grade") or "unverified"))
    except ValueError:
        grade = EvidenceGrade.UNVERIFIED
    try:
        severity = ClaimSeverity(str(item.get("severity") or "none"))
    except ValueError:
        severity = ClaimSeverity.NONE
    severity = clamp_claim_severity(grade, severity)

    title = str(item.get("title") or "").strip()[:300]
    if not title:
        return None

    try:
        return Finding(
            engagement_id=engagement_id,
            run_id=run_id,
            phase=phase,
            finding_type=ftype,
            title=title,
            description=str(item.get("description") or "").strip()[:500],
            evidence=str(item.get("evidence") or "").strip()[:500],
            confidence=FindingConfidence.LIKELY,
            evidence_grade=grade,
            claim_severity=severity,
            source_tool=tool_name,
            target=target,
            tags=["dynamic_parsed", f"grade:{grade.value}"],
        )
    except ValidationError:
        return None

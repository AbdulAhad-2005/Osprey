...
def _first(value: Any) -> str:
    """First element of a list, or the value itself, as a stripped string."""
...
def parse_nuclei(
    ...
    *,
    ...
    for line in (stdout or "").splitlines():
        ...
        template = str(rec.get("template-id") or rec.get("templateID") or "").strip()
        name = str(info.get("name") or template or "nuclei finding").strip()
        severity = _sev(info.get("severity"))
        matched = str(rec.get("matched-at") or rec.get("matched") or rec.get("host") or target).strip()
        classification = info.get("classification") or {}
        cve = _first(classification.get("cve-id"))
        cwe = _first(classification.get("cwe-id"))
    ...
    return out or _unparsed(stdout, tool="nuclei_scan", engagement_id=engagement_id, run_id=run_id, target=target)


# --------------------------------------------------------------------------- #
# nikto — text ("+ " finding lines)
# --------------------------------------------------------------------------- #

_NIKTO_OSVDB_RE = re.compile(r"OSVDB-(\d+)")
    ...
    *,
    ...
    for line in (stdout or "").splitlines():
        ...
        if not text.startswith("+ "):
            continue
        body = text[2:].strip()
        # Skip banner/summary lines that aren't findings.
        low = body.lower()
        if low.startswith(("target ip", "target host", "target port", "start time", "end time", "server:", "ssl info", "root page", "no cgi", "scan terminated", "host(s) tested", "requests:")):
            continue
    ...
    return out or _unparsed(stdout, tool="nikto_scan", engagement_id=engagement_id, run_id=run_id, target=target)
    ...
    *,
    ...
    for item in data.get("interesting_findings") or []:
        if not isinstance(item, dict):
            continue
        url = str(item.get("url") or tgt)
        desc = _first(item.get("to_s")) or _first(item.get("type"))
        if not desc:
            continue
        out.append(
            _vuln_finding(
    ...
    return out or _unparsed(stdout, tool="wpscan_analyze", engagement_id=engagement_id, run_id=run_id, target=target)
...
def parse_sqlmap(
    ...
    *,
    ...
    injectable = (
        ...
        or "the following injection point" in low
        or "sqlmap identified the following injection" in low
        or bool(_SQLMAP_PARAM_RE.search(text))
    )
    if not injectable:
...
# --------------------------------------------------------------------------- #
# jaeles — text ("[Severity][signature] url")
# --------------------------------------------------------------------------- #

_JAELES_RE = re.compile(r"\[(?P<sev>critical|high|medium|low|info)\]\s*\[(?P<sig>[^\]]+)\][^\n]*?(?P<url>https?://\S+)", re.IGNORECASE)
...
def parse_jaeles(
    ...
    *,
    ...
    return out or _unparsed(stdout, tool="jaeles_vulnerability_scan", engagement_id=engagement_id, run_id=run_id, target=target)
...
def parse_searchsploit_lookup(
    ...
    for entry in (data.get("RESULTS_EXPLOIT") or []) + (data.get("RESULTS_SHELLCODE") or []):
        ...
        edb_id = str(entry.get("EDB-ID") or "").strip()
    ...
    return out or _unparsed(stdout, tool="searchsploit_lookup", engagement_id=engagement_id, run_id=run_id, target=target)


# --------------------------------------------------------------------------- #
# sslyze — TLS/SSL deep scan (JSON, --json_out=-)
# --------------------------------------------------------------------------- #
...
def _sslyze_scan_block(scan_result: dict, command: str) -> dict:
    block = scan_result.get(command) or {}
...
def parse_sslyze(
    ...
    *,
    ...
    for server in data.get("server_scan_results") or []:
        ...
        tgt = f"{host}:{port}" if host else target
        scan = server.get("scan_result") or {}

        # 1) Weak protocol versions enabled
        ...
        # 2) Weak cipher suites (any protocol)
        weak_seen: set[str] = set()
        for cmd in _ALL_CIPHER_SCANS:
            for entry in (_sslyze_scan_block(scan, cmd) or {}).get("accepted_cipher_suites") or []:
                cs = entry.get("cipher_suite") if isinstance(entry, dict) else {}
                name = str((cs or {}).get("name") or (cs or {}).get("openssl_name") or "")
                if not name:
                    continue
    ...
    return out or _unparsed(stdout, tool="sslyze_scan", engagement_id=engagement_id, run_id=run_id, target=target)


# --------------------------------------------------------------------------- #
# graphql-cop — GraphQL security checks (JSON, -o json)
# --------------------------------------------------------------------------- #

def parse_graphql_cop(
    stdout: str,
    *,
    engagement_id: str = "",
    run_id: str = "",
    ...
    try:
        data = json.loads(text[start:] if start != -1 and (obj_start == -1 or start < obj_start) else text[obj_start:])
    except json.JSONDecodeError:
        return _unparsed(stdout, tool="graphql_cop_scan", engagement_id=engagement_id, run_id=run_id, target=target)
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        title = str(item.get("title") or item.get("name") or "GraphQL issue").strip()
        # graphql-cop marks a check's result; only report ones that fired.
        result_flag = item.get("result")
    ...
    return out or _unparsed(stdout, tool="graphql_cop_scan", engagement_id=engagement_id, run_id=run_id, target=target)
...
def _vuln_digest(tool: str, findings: list[Finding]) -> str:
    ...
    parts = [f"{by_sev[s]} {s}" for s in order if by_sev.get(s)]
    return f"{tool}: {sum(by_sev.values())} finding(s) — " + ", ".join(parts)


def _make_digester(tool: str, parser):
    def _d(stdout: str) -> str:
        try:
            return _vuln_digest(tool, parser(stdout))
        except Exception:
            return ""
    return _d


def _register_vuln_parsers() -> None:
    ...
    for tool, parser in (
        ...
        ("nikto_scan", parse_nikto),
        ("wpscan_analyze", parse_wpscan),
        ("sqlmap_scan", parse_sqlmap),
        ("dalfox_xss_scan", parse_dalfox),
        ("jaeles_vulnerability_scan", parse_jaeles),
        ("searchsploit_lookup", parse_searchsploit_lookup),
        ("sslyze_scan", parse_sslyze),
        ("graphql_cop_scan", parse_graphql_cop),
    ):
        register_output_parser(tool, parser)
        register_output_digester(tool, _make_digester(tool, parser))


_register_vuln_parsers()

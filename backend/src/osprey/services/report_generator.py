"""Structured report data engine — metrics, findings, infrastructure, topology, next steps.

Provides complete structured data an LLM needs to write a pentest report.
"""
...
_REMEDIATION_HINTS = {
    ...
    "eol": "Upgrade software stack to a actively supported version to receive security patches.",
...
def build_report_data(engagement_id: str) -> dict[str, Any]:
    """Aggregate engagement data into a structured report payload."""
    ...
    return {
        "engagement_id": eid,
        "metrics": _build_metrics(findings, nodes, edges),
        "severity_breakdown": _build_severity_breakdown(findings),
        "findings_by_severity": _build_findings_by_severity(findings),
        "infrastructure_notes": _build_infra_notes(findings, nodes, edges),
        "technologies_detected": _build_tech_summary(findings, nodes),
...
def _build_findings_by_severity(findings: list) -> list[dict[str, Any]]:
    """Findings grouped and sorted by severity, most severe first."""
...
def _build_infra_notes(
    findings: list,
    nodes: list,
    edges: list,
) -> dict[str, Any]:
    """Extract infrastructure posture from existing findings."""
    waf_detected: list[str] = []
    cloudflare: list[str] = []
    techs = set()
    email_posture: list[str] = []
    ...
    for f in findings:
        ...
        # WAF detection
        if "waf" in tags or "waf_detected" in tags:
            waf_detected.append(f.target or f.title)
        if meta.get("is_cloudflare") or meta.get("cloudflare") or "cloudflare" in tags:
            cloudflare.append(meta.get("hostname") or f.target or f.title)
        ...
        # Email security
        if f.finding_type.value == "observation" and "email" in str(meta).lower():
            email_posture.append(f.title)

    return {
        "waf_detected": list(set(waf_detected)),
        "cloudflare_assets": list(set(cloudflare)),
        "email_posture": list(set(email_posture)),
    }


def _build_tech_summary(findings: list, nodes: list) -> list[str]:
    """Deduplicated technology list from findings and graph nodes."""
    techs: set[str] = set()
    for f in findings:

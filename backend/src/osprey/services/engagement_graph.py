"""Engagement graph — Postgres-backed, strictly scoped per engagement.

Same subdomain/IP labels on different engagements never share nodes or edges.
Composite PK is (engagement_id, node_id) where node_id = asset_type:label.
"""

from __future__ import annotations

import logging
import re
import threading
from typing import Any, Iterable

import orjson
from sqlalchemy import or_, select

from osprey.db.session import SessionLocal
from osprey.models.finding import AssetEdgeRow, AssetNodeRow, FindingRow
from osprey.schemas.engagement_graph import (
    AssetEdge,
    AssetNode,
    AssetType,
    GraphSummary,
    SiblingHostResponse,
)
from osprey.schemas.finding import Finding, FindingType

logger = logging.getLogger(__name__)

_IP_RE = re.compile(r"\b(?:\d{1,3}\.){3}\d{1,3}\b")

# Evidence-grade strength ordering (for node provenance strengthening).
_GRADE_RANK = {"unverified": 0, "inferred": 1, "observed": 2}

# Finding types whose primary graph node is keyed directly on the finding title
# (label == title), so provenance can be stamped generically after ingest. Port /
# service / technology node labels are derived differently and are intentionally
# excluded here to avoid mis-keying.
_PRIMARY_NODE_ASSET: dict[FindingType, AssetType] = {
    FindingType.SUBDOMAIN: AssetType.SUBDOMAIN,
    FindingType.HOST: AssetType.HOST,
    FindingType.URL: AssetType.URL,
    FindingType.EMAIL: AssetType.EMAIL,
    FindingType.USERNAME: AssetType.USERNAME,
    FindingType.PERSON: AssetType.PERSON,
    FindingType.PHONE: AssetType.PHONE,
    FindingType.SOCIAL_ACCOUNT: AssetType.SOCIAL_ACCOUNT,
    FindingType.ORGANIZATION: AssetType.ORGANIZATION,
    FindingType.DOCUMENT: AssetType.DOCUMENT,
}


def _node_id(asset_type: AssetType, label: str) -> str:
    """Local node key within an engagement (engagement_id is a separate PK part)."""
    return f"{asset_type.value}:{label.lower()}"


def _encode_meta(meta: dict) -> str:
    return orjson.dumps(meta).decode()


def _decode_meta(raw: str) -> dict:
    if not raw:
        return {}
    try:
        data = orjson.loads(raw)
        return data if isinstance(data, dict) else {}
    except (orjson.JSONDecodeError, TypeError, ValueError):
        return {}


class EngagementGraph:
    def __init__(self) -> None:
        self._lock = threading.Lock()

    def ingest_finding(self, finding: Finding) -> None:
        self.ingest_many([finding])

    def ingest_many(self, findings: Iterable[Finding]) -> None:
        items = list(findings)
        if not items:
            return

        with self._lock:
            db = SessionLocal()
            try:
                for finding in items:
                    # Per-finding savepoint: a duplicate edge / bad row rolls back
                    # only THAT finding, never the whole batch. Previously one
                    # IntegrityError (e.g. a script printing the same edge twice)
                    # poisoned the session and discarded every finding in the run.
                    sp = db.begin_nested()
                    try:
                        self._ingest_one(db, finding)
                        self._stamp_primary_provenance(db, finding)
                        sp.commit()
                    except Exception as exc:  # noqa: BLE001
                        sp.rollback()
                        logger.debug("graph ingest skipped one finding: %s", exc)
                db.commit()
            except Exception:
                db.rollback()
                raise
            finally:
                db.close()

    def _stamp_primary_provenance(self, db, finding: Finding) -> None:
        """Record which tool observed a finding's own asset node, and its grade.

        Generic (one place) so the 20 node-construction sites in ``_ingest_one`` stay
        untouched. Only types whose node label == finding title are stamped.
        """
        asset_type = _PRIMARY_NODE_ASSET.get(finding.finding_type)
        if asset_type is None:
            return
        label = (finding.title or "").strip()
        if not label:
            return
        node_id = _node_id(asset_type, label)
        row = db.get(AssetNodeRow, (finding.engagement_id or "", node_id))
        if row is None:
            return
        if finding.source_tool and not row.source_tool:
            row.source_tool = finding.source_tool
        grade = finding.evidence_grade.value
        if _GRADE_RANK.get(grade, 0) > _GRADE_RANK.get(row.evidence_grade or "inferred", 0):
            row.evidence_grade = grade
        row.confidence = finding.confidence.value
        # Link the finding to its primary graph node (findings_store has already
        # committed the canonical row; a merged occurrence's id won't resolve here,
        # and the canonical was linked on its first sighting).
        frow = db.get(FindingRow, finding.id)
        if frow is not None and not frow.node_id:
            frow.node_id = node_id

    def _upsert_node(self, db, node: AssetNode) -> AssetNode:
        eid = node.engagement_id or ""
        existing = db.get(AssetNodeRow, (eid, node.id))
        if existing is None:
            # Pending unflushed inserts in this session (e.g. same IP on second host)
            for obj in list(db.new):
                if (
                    isinstance(obj, AssetNodeRow)
                    and obj.engagement_id == eid
                    and obj.id == node.id
                ):
                    existing = obj
                    break
        if existing:
            merged = {**_decode_meta(existing.metadata_json), **(node.metadata or {})}
            existing.metadata_json = _encode_meta(merged)
            existing.run_id = node.run_id or existing.run_id
            existing.label = node.label
            existing.asset_type = node.asset_type.value
            # Provenance: keep the first tool that saw the node; strengthen the grade
            # only when a later observation is strictly stronger.
            if node.source_tool and not existing.source_tool:
                existing.source_tool = node.source_tool
            if _GRADE_RANK.get(node.evidence_grade, 0) > _GRADE_RANK.get(existing.evidence_grade or "inferred", 0):
                existing.evidence_grade = node.evidence_grade
            return AssetNode(
                id=existing.id,
                asset_type=AssetType(existing.asset_type),
                label=existing.label,
                engagement_id=existing.engagement_id,
                run_id=existing.run_id,
                source_tool=existing.source_tool,
                evidence_grade=existing.evidence_grade,
                confidence=existing.confidence,
                metadata=merged,
            )

        row = AssetNodeRow(
            engagement_id=eid,
            id=node.id,
            asset_type=node.asset_type.value,
            label=node.label,
            run_id=node.run_id or "",
            source_tool=node.source_tool or "",
            evidence_grade=node.evidence_grade or "inferred",
            confidence=node.confidence or "confirmed",
            metadata_json=_encode_meta(node.metadata or {}),
        )
        db.add(row)
        return node

    def _add_edge(self, db, edge: AssetEdge) -> None:
        if edge.source_id == edge.target_id:
            return
        eid = edge.engagement_id or ""
        exists = db.scalars(
            select(AssetEdgeRow).where(
                AssetEdgeRow.engagement_id == eid,
                AssetEdgeRow.source_id == edge.source_id,
                AssetEdgeRow.target_id == edge.target_id,
                AssetEdgeRow.relationship == edge.relationship,
            )
        ).first()
        if exists:
            return
        db.add(
            AssetEdgeRow(
                engagement_id=eid,
                source_id=edge.source_id,
                target_id=edge.target_id,
                relationship=edge.relationship,
                run_id=edge.run_id or "",
                source_tool=edge.source_tool or "",
                metadata_json=_encode_meta(edge.metadata or {}),
            )
        )

    def _link_resolves_and_cohosts(
        self,
        db,
        *,
        host_node_id: str,
        ip: str,
        eid: str,
        rid: str,
    ) -> None:
        """Create resolves_to and soft co_hosts edges for other hosts on the same IP."""
        if not ip:
            return
        ip_node = self._upsert_node(
            db,
            AssetNode(
                id=_node_id(AssetType.IP, ip),
                asset_type=AssetType.IP,
                label=ip,
                engagement_id=eid,
                run_id=rid,
            ),
        )
        self._add_edge(
            db,
            AssetEdge(
                source_id=host_node_id,
                target_id=ip_node.id,
                relationship="resolves_to",
                engagement_id=eid,
                run_id=rid,
            ),
        )
        # Flush so peer lookup sees resolves_to edges created earlier in this batch
        db.flush()
        peers = db.scalars(
            select(AssetEdgeRow).where(
                AssetEdgeRow.engagement_id == eid,
                AssetEdgeRow.target_id == ip_node.id,
                AssetEdgeRow.relationship == "resolves_to",
            )
        ).all()
        for peer in peers:
            if peer.source_id == host_node_id:
                continue
            self._add_edge(
                db,
                AssetEdge(
                    source_id=host_node_id,
                    target_id=peer.source_id,
                    relationship="co_hosts",
                    engagement_id=eid,
                ),
            )
            self._add_edge(
                db,
                AssetEdge(
                    source_id=peer.source_id,
                    target_id=host_node_id,
                    relationship="co_hosts",
                    engagement_id=eid,
                ),
            )

    def _is_sister_finding(self, finding: Finding) -> bool:
        if "sister_domain" in (finding.tags or []):
            return True
        if finding.source_tool == "domain_hunter":
            return True
        role = str((finding.metadata or {}).get("role", "")).lower()
        return role in ("sister_domain", "sister", "affiliated")

    def _osint_node(
        self,
        db,
        *,
        atype: AssetType,
        label: str,
        eid: str,
        rid: str,
        meta: dict | None = None,
    ) -> AssetNode:
        """Upsert a passive-OSINT identity node (email/username/person/…)."""
        return self._upsert_node(
            db,
            AssetNode(
                id=_node_id(atype, label),
                asset_type=atype,
                label=label,
                engagement_id=eid,
                run_id=rid,
                metadata=meta or {},
            ),
        )

    def _resolve_or_seed_domain(self, db, *, seed: str, eid: str, rid: str) -> AssetNode:
        """Attach lineage edges (subdomain_of / affiliated_with) to an existing
        node for `seed` rather than always minting a fresh DOMAIN placeholder.

        A scan target that is itself a subdomain (e.g. a recursively-discovered
        host like apnegn.geo.tv) already exists as a SUBDOMAIN node, fully
        tracked by surface_expansion.py's frontier. Node identity there is
        (asset_type, label) — so unconditionally creating a second DOMAIN node
        for the same label produces a permanent duplicate that never gets an
        `expanded` flag (nothing in this "seed skeleton" path sets one) and
        re-enters the discovery frontier every single pass forever, since its
        SUBDOMAIN sibling converging never touches this node at all.
        """
        existing = db.get(AssetNodeRow, (eid, _node_id(AssetType.SUBDOMAIN, seed)))
        if existing is not None:
            return AssetNode(
                id=existing.id,
                asset_type=AssetType.SUBDOMAIN,
                label=existing.label,
                engagement_id=existing.engagement_id,
                run_id=existing.run_id,
                source_tool=existing.source_tool,
                evidence_grade=existing.evidence_grade,
                confidence=existing.confidence,
                metadata=_decode_meta(existing.metadata_json),
            )
        return self._upsert_node(
            db,
            AssetNode(
                id=_node_id(AssetType.DOMAIN, seed),
                asset_type=AssetType.DOMAIN,
                label=seed,
                engagement_id=eid,
                run_id=rid,
                metadata={"role": "seed"},
            ),
        )

    def _osint_edge(self, db, *, src: str, tgt: str, rel: str, eid: str) -> None:
        self._add_edge(
            db,
            AssetEdge(source_id=src, target_id=tgt, relationship=rel, engagement_id=eid),
        )

    def _ingest_one(self, db, finding: Finding) -> None:
        eid = finding.engagement_id or ""
        rid = finding.run_id or ""
        label = finding.title.strip()
        if not label:
            return

        if finding.finding_type == FindingType.SUBDOMAIN:
            meta = dict(finding.metadata)
            node = self._upsert_node(
                db,
                AssetNode(
                    id=_node_id(AssetType.SUBDOMAIN, label),
                    asset_type=AssetType.SUBDOMAIN,
                    label=label,
                    engagement_id=eid,
                    run_id=rid,
                    metadata=meta,
                ),
            )
            ip = str(meta.get("ip", "") or "")
            if not ip:
                ip_match = _IP_RE.search(finding.evidence)
                ip = ip_match.group(0) if ip_match else ""
            self._link_resolves_and_cohosts(db, host_node_id=node.id, ip=ip, eid=eid, rid=rid)

            # Auto-skeleton: subdomain -> subdomain_of -> parent domain. Parsers pass
            # the enumerated root as `target`; this is deterministic structure, not
            # LLM cognition, so the platform builds it without being asked.
            seed = (finding.target or "").strip().lower()
            if seed and seed != label.lower() and label.lower().endswith("." + seed):
                domain_node = self._resolve_or_seed_domain(db, seed=seed, eid=eid, rid=rid)
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=node.id,
                        target_id=domain_node.id,
                        relationship="subdomain_of",
                        engagement_id=eid,
                    ),
                )

        elif finding.finding_type == FindingType.URL:
            meta = dict(finding.metadata)
            self._upsert_node(
                db,
                AssetNode(
                    id=_node_id(AssetType.URL, label),
                    asset_type=AssetType.URL,
                    label=label,
                    engagement_id=eid,
                    run_id=rid,
                    metadata=meta,
                ),
            )
            host = label.split("//")[-1].split("/")[0].split(":")[0]
            if host:
                host_meta = {"hostname": host}
                if meta.get("is_cloudflare"):
                    host_meta["is_cloudflare"] = True
                    host_meta["waf"] = str(meta.get("waf") or "cloudflare")
                host_node = self._upsert_node(
                    db,
                    AssetNode(
                        id=_node_id(AssetType.HOST, host),
                        asset_type=AssetType.HOST,
                        label=host,
                        engagement_id=eid,
                        run_id=rid,
                        metadata=host_meta,
                    ),
                )
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=_node_id(AssetType.URL, label),
                        target_id=host_node.id,
                        relationship="hosted_on",
                        engagement_id=eid,
                    ),
                )

        elif finding.finding_type == FindingType.PORT:
            host = finding.target or label.split(":")[0]
            port = str(finding.metadata.get("port", ""))
            port_label = f"{host}:{port}" if port else label
            self._upsert_node(
                db,
                AssetNode(
                    id=_node_id(AssetType.PORT, port_label),
                    asset_type=AssetType.PORT,
                    label=port_label,
                    engagement_id=eid,
                    run_id=rid,
                    metadata=dict(finding.metadata),
                ),
            )
            if host:
                self._upsert_node(
                    db,
                    AssetNode(
                        id=_node_id(AssetType.HOST, host),
                        asset_type=AssetType.HOST,
                        label=host,
                        engagement_id=eid,
                        run_id=rid,
                    ),
                )
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=_node_id(AssetType.HOST, host),
                        target_id=_node_id(AssetType.PORT, port_label),
                        relationship="has_port",
                        engagement_id=eid,
                    ),
                )

        elif finding.finding_type == FindingType.SERVICE:
            service_node = self._upsert_node(
                db,
                AssetNode(
                    id=_node_id(AssetType.SERVICE, label),
                    asset_type=AssetType.SERVICE,
                    label=label,
                    engagement_id=eid,
                    run_id=rid,
                    metadata=dict(finding.metadata),
                ),
            )
            # Auto-skeleton: a SERVICE finding still describes a host:port — build
            # the same host->has_port->port structure the PORT branch builds, then
            # attach the service so it is never an orphan node in the graph.
            host = finding.target or label.split(":")[0]
            port = str(finding.metadata.get("port", ""))
            if host and port:
                port_label = f"{host}:{port}"
                self._upsert_node(
                    db,
                    AssetNode(
                        id=_node_id(AssetType.HOST, host),
                        asset_type=AssetType.HOST,
                        label=host,
                        engagement_id=eid,
                        run_id=rid,
                    ),
                )
                port_node = self._upsert_node(
                    db,
                    AssetNode(
                        id=_node_id(AssetType.PORT, port_label),
                        asset_type=AssetType.PORT,
                        label=port_label,
                        engagement_id=eid,
                        run_id=rid,
                        metadata=dict(finding.metadata),
                    ),
                )
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=_node_id(AssetType.HOST, host),
                        target_id=port_node.id,
                        relationship="has_port",
                        engagement_id=eid,
                    ),
                )
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=port_node.id,
                        target_id=service_node.id,
                        relationship="runs_service",
                        engagement_id=eid,
                    ),
                )

        elif finding.finding_type == FindingType.TECHNOLOGY:
            tech = label or str(finding.metadata.get("technology", ""))
            if tech:
                tech_node = self._upsert_node(
                    db,
                    AssetNode(
                        id=_node_id(AssetType.TECHNOLOGY, tech),
                        asset_type=AssetType.TECHNOLOGY,
                        label=tech,
                        engagement_id=eid,
                        run_id=rid,
                        metadata=dict(finding.metadata),
                    ),
                )
                # Auto-skeleton: attach the tech to the host it was fingerprinted on
                # (parsers pass hostname in metadata, or target is a hostname/URL).
                host = str(finding.metadata.get("hostname", "") or "")
                if not host:
                    candidate = (finding.target or "").strip()
                    if candidate.lower().startswith(("http://", "https://")):
                        host = candidate.split("//", 1)[-1].split("/")[0].split(":")[0]
                    elif candidate and " " not in candidate and "." in candidate:
                        host = candidate
                if host:
                    host_node = self._upsert_node(
                        db,
                        AssetNode(
                            id=_node_id(AssetType.HOST, host),
                            asset_type=AssetType.HOST,
                            label=host,
                            engagement_id=eid,
                            run_id=rid,
                        ),
                    )
                    self._add_edge(
                        db,
                        AssetEdge(
                            source_id=host_node.id,
                            target_id=tech_node.id,
                            relationship="runs_tech",
                            engagement_id=eid,
                        ),
                    )

        elif finding.finding_type == FindingType.HOST:
            meta = dict(finding.metadata)
            if self._is_sister_finding(finding):
                meta.setdefault("role", "sister_domain")
            host_node = self._upsert_node(
                db,
                AssetNode(
                    id=_node_id(AssetType.HOST, label),
                    asset_type=AssetType.HOST,
                    label=label,
                    engagement_id=eid,
                    run_id=rid,
                    metadata=meta,
                ),
            )
            # Sister lineage: seed domain → affiliated_with → sister host
            seed = (finding.target or "").strip().lower()
            if seed and self._is_sister_finding(finding) and seed != label.lower():
                seed_node = self._resolve_or_seed_domain(db, seed=seed, eid=eid, rid=rid)
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=seed_node.id,
                        target_id=host_node.id,
                        relationship="affiliated_with",
                        engagement_id=eid,
                    ),
                )
            ip = str(meta.get("ip", "") or "")
            if not ip:
                ip_match = _IP_RE.search(finding.evidence or "")
                ip = ip_match.group(0) if ip_match else ""
            self._link_resolves_and_cohosts(db, host_node_id=host_node.id, ip=ip, eid=eid, rid=rid)

            # CDN-origin lineage: fronted host → origin_of → discovered origin IP.
            # cdn_origin_probe tags its candidates role=="origin_candidate" with
            # `finding.target` set to the CDN-fronted host it probed — so the
            # surface-expansion frontier can tell "this is a re-scan-worthy real
            # origin" from "this is the CDN edge IP" without re-deriving it.
            if meta.get("role") == "origin_candidate" and seed and seed != label.lower():
                fronted_node = self._upsert_node(
                    db,
                    AssetNode(
                        id=_node_id(AssetType.HOST, seed),
                        asset_type=AssetType.HOST,
                        label=seed,
                        engagement_id=eid,
                        run_id=rid,
                        metadata={"behind_cdn": True},
                    ),
                )
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=fronted_node.id,
                        target_id=host_node.id,
                        relationship="origin_of",
                        engagement_id=eid,
                        metadata={"confidence": meta.get("confidence"), "signals": meta.get("signals", "")},
                    ),
                )

        # ── Passive OSINT identity entities ──────────────────────────────────
        # Parsers pass relationship hints in metadata (person / domain / username /
        # organization / network). We build the entity node and, when a hint is
        # present, the pivot edge — so name → email → account chains are queryable.
        elif finding.finding_type == FindingType.EMAIL:
            addr = label.lower()
            meta = dict(finding.metadata)
            email_node = self._osint_node(
                db, atype=AssetType.EMAIL, label=addr, eid=eid, rid=rid, meta=meta
            )
            if "@" in addr:
                dom = addr.split("@")[-1].strip()
                if dom and "." in dom:
                    dom_node = self._osint_node(
                        db, atype=AssetType.DOMAIN, label=dom, eid=eid, rid=rid,
                        meta={"role": "email_domain"},
                    )
                    self._osint_edge(db, src=dom_node.id, tgt=email_node.id, rel="has_email", eid=eid)
            person = str(meta.get("person", "") or "").strip()
            if person:
                p_node = self._osint_node(
                    db, atype=AssetType.PERSON, label=person, eid=eid, rid=rid
                )
                self._osint_edge(db, src=p_node.id, tgt=email_node.id, rel="owns_email", eid=eid)

        elif finding.finding_type == FindingType.USERNAME:
            meta = dict(finding.metadata)
            u_node = self._osint_node(
                db, atype=AssetType.USERNAME, label=label, eid=eid, rid=rid, meta=meta
            )
            person = str(meta.get("person", "") or "").strip()
            if person:
                p_node = self._osint_node(
                    db, atype=AssetType.PERSON, label=person, eid=eid, rid=rid
                )
                self._osint_edge(db, src=p_node.id, tgt=u_node.id, rel="uses_username", eid=eid)

        elif finding.finding_type == FindingType.PERSON:
            meta = dict(finding.metadata)
            p_node = self._osint_node(
                db, atype=AssetType.PERSON, label=label, eid=eid, rid=rid, meta=meta
            )
            org = str(meta.get("organization", "") or "").strip()
            if org:
                org_node = self._osint_node(
                    db, atype=AssetType.ORGANIZATION, label=org, eid=eid, rid=rid
                )
                self._osint_edge(db, src=p_node.id, tgt=org_node.id, rel="works_at", eid=eid)

        elif finding.finding_type == FindingType.PHONE:
            meta = dict(finding.metadata)
            ph_node = self._osint_node(
                db, atype=AssetType.PHONE, label=label, eid=eid, rid=rid, meta=meta
            )
            person = str(meta.get("person", "") or "").strip()
            if person:
                p_node = self._osint_node(
                    db, atype=AssetType.PERSON, label=person, eid=eid, rid=rid
                )
                self._osint_edge(db, src=p_node.id, tgt=ph_node.id, rel="has_phone", eid=eid)

        elif finding.finding_type == FindingType.SOCIAL_ACCOUNT:
            meta = dict(finding.metadata)
            sa_node = self._osint_node(
                db, atype=AssetType.SOCIAL_ACCOUNT, label=label, eid=eid, rid=rid, meta=meta
            )
            uname = str(meta.get("username", "") or "").strip()
            if uname:
                u_node = self._osint_node(
                    db, atype=AssetType.USERNAME, label=uname, eid=eid, rid=rid
                )
                self._osint_edge(db, src=u_node.id, tgt=sa_node.id, rel="used_on", eid=eid)
            person = str(meta.get("person", "") or "").strip()
            if person:
                p_node = self._osint_node(
                    db, atype=AssetType.PERSON, label=person, eid=eid, rid=rid
                )
                self._osint_edge(db, src=sa_node.id, tgt=p_node.id, rel="belongs_to", eid=eid)

        elif finding.finding_type == FindingType.ORGANIZATION:
            meta = dict(finding.metadata)
            org_node = self._osint_node(
                db, atype=AssetType.ORGANIZATION, label=label, eid=eid, rid=rid, meta=meta
            )
            dom = str(meta.get("domain", "") or "").strip().lower()
            if dom and "." in dom:
                dom_node = self._osint_node(
                    db, atype=AssetType.DOMAIN, label=dom, eid=eid, rid=rid
                )
                self._osint_edge(db, src=org_node.id, tgt=dom_node.id, rel="owns_domain", eid=eid)

        elif finding.finding_type == FindingType.DOCUMENT:
            meta = dict(finding.metadata)
            doc_node = self._osint_node(
                db, atype=AssetType.DOCUMENT, label=label, eid=eid, rid=rid, meta=meta
            )
            dom = str(meta.get("domain", "") or "").strip().lower()
            if dom and "." in dom:
                dom_node = self._osint_node(
                    db, atype=AssetType.DOMAIN, label=dom, eid=eid, rid=rid
                )
                self._osint_edge(db, src=dom_node.id, tgt=doc_node.id, rel="exposes_document", eid=eid)
            author = str(meta.get("person", "") or meta.get("author", "") or "").strip()
            if author:
                p_node = self._osint_node(
                    db, atype=AssetType.PERSON, label=author, eid=eid, rid=rid
                )
                self._osint_edge(db, src=doc_node.id, tgt=p_node.id, rel="authored_by", eid=eid)

        elif finding.finding_type == FindingType.DNS_RECORD:
            # MX/NS/CNAME carry a hostname value -> make it a real HOST node and
            # link it to the record's owner (has_mx_record / has_ns_record /
            # cname_to), so mail and nameserver infra is queryable in the graph
            # instead of buried in a flat observation. TXT/SPF/etc. are strings,
            # not assets — the finding itself carries them, no node.
            meta = dict(finding.metadata)
            rtype = str(meta.get("record_type", "") or "").lower()
            owner = str(meta.get("hostname", "") or "").strip().lower().rstrip(".")
            value = str(meta.get(rtype) or meta.get("value") or "").strip().lower().rstrip(".")
            rel = {"mx": "has_mx_record", "ns": "has_ns_record", "cname": "cname_to"}.get(rtype)
            if owner and value and rel:
                owner_node = self._osint_node(
                    db, atype=self._existing_or_guess_host_type(db, owner, eid),
                    label=owner, eid=eid, rid=rid, meta={"role": "dns_owner"},
                )
                val_node = self._osint_node(
                    db, atype=AssetType.HOST, label=value, eid=eid, rid=rid,
                    meta={"role": f"{rtype}_target"},
                )
                self._osint_edge(db, src=owner_node.id, tgt=val_node.id, rel=rel, eid=eid)

        elif finding.finding_type in (FindingType.CREDENTIAL, FindingType.SECRET):
            # A credential/secret is only useful if you know where it belongs —
            # link it to the host/domain it was found on so the graph shows the
            # exposure, not an orphaned string.
            meta = dict(finding.metadata)
            atype = (
                AssetType.CREDENTIAL
                if finding.finding_type == FindingType.CREDENTIAL
                else AssetType.SECRET
            )
            cred_node = self._osint_node(db, atype=atype, label=label, eid=eid, rid=rid, meta=meta)
            host = str(
                meta.get("hostname") or meta.get("host") or meta.get("ip") or finding.target or ""
            ).strip().lower()
            # target may be a full URL (js_recon scans a URL) — reduce to host.
            if "://" in host or "/" in host:
                from urllib.parse import urlparse

                parsed = urlparse(host if "://" in host else f"//{host}")
                host = (parsed.netloc or parsed.path.split("/")[0]).split("@")[-1].split(":")[0]
            host = host.rstrip(".")
            if host and "." in host:
                host_node = self._osint_node(
                    db, atype=self._existing_or_guess_host_type(db, host, eid),
                    label=host, eid=eid, rid=rid,
                )
                rel = "exposes_credential" if atype == AssetType.CREDENTIAL else "exposes_secret"
                self._osint_edge(db, src=host_node.id, tgt=cred_node.id, rel=rel, eid=eid)

    def _existing_or_guess_host_type(self, db, label: str, eid: str) -> AssetType:
        """Pick the node type for a hostname: reuse an existing domain/subdomain
        node if one is already in the graph, else infer apex (<=2 labels) vs
        subdomain from depth. Avoids creating a duplicate DOMAIN node for a name
        already tracked as a SUBDOMAIN (and vice versa)."""
        for atype in (AssetType.DOMAIN, AssetType.SUBDOMAIN, AssetType.HOST):
            if db.get(AssetNodeRow, (eid, _node_id(atype, label))):
                return atype
        return AssetType.DOMAIN if label.count(".") <= 1 else AssetType.SUBDOMAIN

    def siblings_same_ip(self, host_or_subdomain: str, *, engagement_id: str = "") -> SiblingHostResponse:
        eid = engagement_id or ""
        with self._lock:
            db = SessionLocal()
            try:
                node_stmt = select(AssetNodeRow)
                edge_stmt = select(AssetEdgeRow)
                if eid:
                    node_stmt = node_stmt.where(AssetNodeRow.engagement_id == eid)
                    edge_stmt = edge_stmt.where(AssetEdgeRow.engagement_id == eid)

                nodes = {(r.engagement_id, r.id): r for r in db.scalars(node_stmt).all()}
                edges = list(db.scalars(edge_stmt).all())
            finally:
                db.close()

        ref = host_or_subdomain.lower().strip()
        ref_id: str | None = None
        for prefix in (AssetType.SUBDOMAIN, AssetType.HOST):
            candidate = _node_id(prefix, ref)
            if eid and (eid, candidate) in nodes:
                ref_id = candidate
                break
            for (row_eid, row_id), _row in nodes.items():
                if row_id == candidate and (not eid or row_eid == eid):
                    ref_id = candidate
                    if not eid:
                        eid = row_eid
                    break
            if ref_id:
                break

        ip_ids: set[str] = set()
        if ref_id:
            for edge in edges:
                if (
                    edge.source_id == ref_id
                    and edge.relationship == "resolves_to"
                    and (not eid or edge.engagement_id == eid)
                ):
                    ip_ids.add(edge.target_id)

        siblings: set[str] = set()
        ips: list[str] = []
        for ip_id in ip_ids:
            ip_node = nodes.get((eid, ip_id))
            if ip_node:
                ips.append(ip_node.label)
            for edge in edges:
                if edge.target_id == ip_id and edge.relationship == "resolves_to":
                    if eid and edge.engagement_id != eid:
                        continue
                    sibling_node = nodes.get((edge.engagement_id, edge.source_id))
                    if sibling_node and sibling_node.label.lower() != ref:
                        siblings.add(sibling_node.label)

        ip_str = ips[0] if ips else ""
        return SiblingHostResponse(
            reference=host_or_subdomain,
            ip=ip_str,
            siblings=sorted(siblings),
            hint=(
                f"{len(siblings)} sibling host(s) on IP {ip_str} — shared hosting signal "
                "(hypothesis until you confirm)"
                if siblings
                else "No IP linkage yet — resolve DNS / probe to populate graph"
            ),
        )

    def ensure_node(
        self,
        *,
        engagement_id: str,
        asset_type: AssetType,
        label: str,
        run_id: str = "",
        metadata: dict | None = None,
    ) -> AssetNode:
        """Upsert a node so operator links/tags have an anchor."""
        eid = (engagement_id or "").strip()
        if not eid:
            raise ValueError("engagement_id required")
        lab = (label or "").strip()
        if not lab:
            raise ValueError("label required")
        node = AssetNode(
            id=_node_id(asset_type, lab),
            asset_type=asset_type,
            label=lab,
            engagement_id=eid,
            run_id=run_id or "",
            metadata={str(k): v for k, v in (metadata or {}).items()},
        )
        with self._lock:
            db = SessionLocal()
            try:
                out = self._upsert_node(db, node)
                db.commit()
                return out
            except Exception:
                db.rollback()
                raise
            finally:
                db.close()

    def operator_link(
        self,
        *,
        engagement_id: str,
        source_type: AssetType,
        source_label: str,
        target_type: AssetType,
        target_label: str,
        relationship: str,
        run_id: str = "",
        source_tool: str = "",
        metadata: dict | None = None,
    ) -> dict[str, str]:
        """Create an operator-named edge between two assets (nodes upserted)."""
        eid = (engagement_id or "").strip()
        if not eid:
            raise ValueError("engagement_id required")
        rel = (relationship or "").strip()
        if not rel or len(rel) > 64:
            raise ValueError("relationship must be 1–64 chars")
        src_lab = (source_label or "").strip()
        tgt_lab = (target_label or "").strip()
        if not src_lab or not tgt_lab:
            raise ValueError("source and target labels required")

        src = AssetNode(
            id=_node_id(source_type, src_lab),
            asset_type=source_type,
            label=src_lab,
            engagement_id=eid,
            run_id=run_id or "",
        )
        tgt = AssetNode(
            id=_node_id(target_type, tgt_lab),
            asset_type=target_type,
            label=tgt_lab,
            engagement_id=eid,
            run_id=run_id or "",
        )
        with self._lock:
            db = SessionLocal()
            try:
                self._upsert_node(db, src)
                self._upsert_node(db, tgt)
                self._add_edge(
                    db,
                    AssetEdge(
                        source_id=src.id,
                        target_id=tgt.id,
                        relationship=rel,
                        engagement_id=eid,
                        run_id=run_id or "",
                        source_tool=source_tool or "",
                        metadata=metadata or {},
                    ),
                )
                db.commit()
            except Exception:
                db.rollback()
                raise
            finally:
                db.close()
        return {
            "source_id": src.id,
            "target_id": tgt.id,
            "relationship": rel,
            "engagement_id": eid,
        }

    def operator_link_many(
        self,
        *,
        engagement_id: str,
        edges: list[dict],
        run_id: str = "",
    ) -> list[dict[str, str]]:
        """Create many operator-named edges in one transaction (nodes upserted).

        Each item in ``edges`` is a dict already validated/sanitized by the caller:
        {source_type, source_label, target_type, target_label, relationship}.
        One commit for the whole batch — this is what makes bulk persistence cheap
        enough that an LLM will actually do it instead of skipping it.
        """
        eid = (engagement_id or "").strip()
        if not eid:
            raise ValueError("engagement_id required")
        if not edges:
            return []

        written: list[dict[str, str]] = []
        with self._lock:
            db = SessionLocal()
            try:
                for item in edges:
                    src_type: AssetType = item["source_type"]
                    tgt_type: AssetType = item["target_type"]
                    src_lab = str(item["source_label"]).strip()
                    tgt_lab = str(item["target_label"]).strip()
                    rel = str(item["relationship"]).strip()
                    if not src_lab or not tgt_lab or not rel:
                        continue

                    src = AssetNode(
                        id=_node_id(src_type, src_lab),
                        asset_type=src_type,
                        label=src_lab,
                        engagement_id=eid,
                        run_id=run_id or "",
                    )
                    tgt = AssetNode(
                        id=_node_id(tgt_type, tgt_lab),
                        asset_type=tgt_type,
                        label=tgt_lab,
                        engagement_id=eid,
                        run_id=run_id or "",
                    )
                    self._upsert_node(db, src)
                    self._upsert_node(db, tgt)
                    self._add_edge(
                        db,
                        AssetEdge(
                            source_id=src.id,
                            target_id=tgt.id,
                            relationship=rel,
                            engagement_id=eid,
                            run_id=run_id or "",
                            source_tool=str(item.get("source_tool") or ""),
                            metadata=item.get("metadata") or {},
                        ),
                    )
                    written.append(
                        {
                            "source_id": src.id,
                            "target_id": tgt.id,
                            "relationship": rel,
                        }
                    )
                db.commit()
            except Exception:
                db.rollback()
                raise
            finally:
                db.close()
        return written

    def list_nodes(
        self,
        *,
        engagement_id: str = "",
        asset_type: AssetType | str | None = None,
        run_id: str = "",
        q: str | None = None,
        limit: int = 5000,
    ) -> list[AssetNode]:
        type_val = asset_type.value if isinstance(asset_type, AssetType) else asset_type
        with self._lock:
            db = SessionLocal()
            try:
                stmt = select(AssetNodeRow)
                if engagement_id:
                    stmt = stmt.where(AssetNodeRow.engagement_id == engagement_id)
                if type_val:
                    stmt = stmt.where(AssetNodeRow.asset_type == type_val)
                if run_id:
                    stmt = stmt.where(AssetNodeRow.run_id == run_id)
                if q and q.strip():
                    # Server-side substring match (label / id / type) so recall
                    # never pulls the whole node set into Python to filter it.
                    q_pat = f"%{q.strip()}%"
                    stmt = stmt.where(
                        or_(
                            AssetNodeRow.label.ilike(q_pat),
                            AssetNodeRow.id.ilike(q_pat),
                            AssetNodeRow.asset_type.ilike(q_pat),
                        )
                    )
                stmt = stmt.limit(max(1, min(limit, 20_000)))
                rows = list(db.scalars(stmt).all())
            finally:
                db.close()
        return [
            AssetNode(
                id=r.id,
                asset_type=AssetType(r.asset_type),
                label=r.label,
                engagement_id=r.engagement_id,
                run_id=r.run_id,
                source_tool=getattr(r, "source_tool", "") or "",
                evidence_grade=getattr(r, "evidence_grade", "inferred") or "inferred",
                confidence=getattr(r, "confidence", "confirmed") or "confirmed",
                metadata=_decode_meta(r.metadata_json),
                created_at=getattr(r, "created_at", None),
                updated_at=getattr(r, "updated_at", None),
            )
            for r in rows
        ]

    def list_edges(
        self,
        *,
        engagement_id: str = "",
        relationship: str | None = None,
        limit: int = 10_000,
    ) -> list[AssetEdge]:
        with self._lock:
            db = SessionLocal()
            try:
                stmt = select(AssetEdgeRow)
                if engagement_id:
                    stmt = stmt.where(AssetEdgeRow.engagement_id == engagement_id)
                if relationship:
                    stmt = stmt.where(AssetEdgeRow.relationship == relationship)
                stmt = stmt.limit(max(1, min(limit, 50_000)))
                rows = list(db.scalars(stmt).all())
            finally:
                db.close()
        return [
            AssetEdge(
                source_id=r.source_id,
                target_id=r.target_id,
                relationship=r.relationship,
                engagement_id=r.engagement_id,
                run_id=getattr(r, "run_id", "") or "",
                source_tool=getattr(r, "source_tool", "") or "",
                metadata=_decode_meta(getattr(r, "metadata_json", "") or ""),
            )
            for r in rows
        ]

    def find_paths_multi_hop(
        self,
        *,
        engagement_id: str,
        start_node_id: str,
        max_hops: int = 3,
        limit: int = 200,
    ) -> list[dict[str, Any]]:
        """Multi-hop neighbor discovery — one indexed SQL query per hop, not a
        full-graph load into Python (the previous limit on every other graph
        traversal in this file). Edges are treated as undirected: "what
        connects to X" shouldn't depend on which way the edge happened to be
        written, matching how a human pentester reasons about relationships.

        Returns nodes reachable from start_node_id within max_hops, each tagged
        with its shortest hop distance and the edge that first reached it —
        sorted by distance so the closest, most relevant connections come first.
        """
        eid = (engagement_id or "").strip()
        start = (start_node_id or "").strip()
        if not eid or not start:
            return []
        hops = max(1, min(int(max_hops), 6))
        cap = max(1, min(int(limit), 1000))

        visited: dict[str, dict[str, Any]] = {}
        frontier: set[str] = {start}
        with self._lock:
            db = SessionLocal()
            try:
                for hop in range(1, hops + 1):
                    if not frontier or len(visited) >= cap:
                        break
                    rows = db.scalars(
                        select(AssetEdgeRow).where(
                            AssetEdgeRow.engagement_id == eid,
                            or_(
                                AssetEdgeRow.source_id.in_(frontier),
                                AssetEdgeRow.target_id.in_(frontier),
                            ),
                        )
                    ).all()
                    next_frontier: set[str] = set()
                    for r in rows:
                        for a, b in ((r.source_id, r.target_id), (r.target_id, r.source_id)):
                            if a in frontier and b != start and b not in visited:
                                visited[b] = {
                                    "node_id": b,
                                    "hops": hop,
                                    "via_relationship": r.relationship,
                                    "via_from": a,
                                }
                                next_frontier.add(b)
                                if len(visited) >= cap:
                                    break
                        if len(visited) >= cap:
                            break
                    frontier = next_frontier
            finally:
                db.close()
        return sorted(visited.values(), key=lambda v: (v["hops"], v["node_id"]))[:cap]

    def summary(
        self,
        *,
        engagement_id: str = "",
        run_id: str = "",
    ) -> GraphSummary:
        if not (engagement_id or "").strip():
            return GraphSummary(
                engagement_id="",
                run_id=run_id,
                node_count=0,
                edge_count=0,
                pivot_hints=["engagement_id required — call platform_health() to bind session"],
            )
        with self._lock:
            db = SessionLocal()
            try:
                node_stmt = select(AssetNodeRow)
                edge_stmt = select(AssetEdgeRow)
                if engagement_id:
                    node_stmt = node_stmt.where(AssetNodeRow.engagement_id == engagement_id)
                    edge_stmt = edge_stmt.where(AssetEdgeRow.engagement_id == engagement_id)
                node_rows = list(db.scalars(node_stmt).all())
                edge_rows = list(db.scalars(edge_stmt).all())
            finally:
                db.close()

        # NOTE: run_id is NOT used to filter the engagement graph summary.
        # MCP sessions mint a new run_id often; filtering made the graph look empty
        # (nodes=0) even when the engagement had hundreds of assets. Graph is
        # engagement-scoped; run_id remains an audit stamp on new writes.

        subdomains = [n.label for n in node_rows if n.asset_type == AssetType.SUBDOMAIN.value]
        live = [
            n.label
            for n in node_rows
            if n.asset_type in (AssetType.URL.value, AssetType.HOST.value)
        ]
        ports = [n.label for n in node_rows if n.asset_type == AssetType.PORT.value]
        techs = [n.label for n in node_rows if n.asset_type == AssetType.TECHNOLOGY.value]

        pivots: list[str] = []
        if len(subdomains) < 5:
            pivots.append("Gap: thin subdomain inventory (few names in graph)")
        if any("445" in p for p in ports):
            pivots.append("Signal: port 445 present — SMB surface may exist (you choose depth)")
        if any("cdn" in (t or "").lower() or "cloudflare" in (t or "").lower() for t in techs):
            pivots.append(
                "Signal: CDN/WAF tech — edge IPs may not be origin (optional origin hunt)"
            )

        # M5: per-IP network granularity instead of dumb "subs but no ports"
        if engagement_id:
            try:
                from osprey.services.network_surface import build_network_surface

                ns = build_network_surface(engagement_id, run_id=run_id)
                if ns is not None:
                    if ns.unscanned_ips:
                        pivots.append(
                            f"Gap: {ns.unscanned_ips} IP(s) without port evidence"
                        )
                    if ns.ports_without_services:
                        pivots.append(
                            f"Gap: {ns.ports_without_services} IP(s) have ports but no SERVICE findings"
                        )
            except Exception:
                logger.debug("network surface pivot failed", exc_info=True)
        elif len(subdomains) >= 2 and not ports:
            pivots.append("Gap: hosts found without port evidence")

        # Soft M4 signals from edges / metadata (engagement already filtered in rows)
        co_hosts = sum(1 for e in edge_rows if e.relationship == "co_hosts")
        if co_hosts:
            pivots.append(
                f"Signal: {co_hosts // 2} co_hosts link(s) — shared-IP siblings exist"
            )
        affiliated = sum(1 for e in edge_rows if e.relationship == "affiliated_with")
        if affiliated:
            pivots.append(f"Signal: {affiliated} sister/affiliated domain(s) linked from seed")
        cf_nodes = [
            n.label
            for n in node_rows
            if _decode_meta(n.metadata_json).get("is_cloudflare")
        ]
        if cf_nodes:
            sample = ", ".join(cf_nodes[:3])
            pivots.append(f"Signal: Cloudflare/WAF on {sample} — origin hunt is optional")

        return GraphSummary(
            engagement_id=engagement_id,
            run_id=run_id,
            node_count=len(node_rows),
            edge_count=len(edge_rows),
            subdomains=subdomains[:50],
            live_hosts=live[:50],
            open_ports=ports[:50],
            technologies=techs[:30],
            pivot_hints=pivots,
        )


_graph: EngagementGraph | None = None


def get_engagement_graph() -> EngagementGraph:
    global _graph
    if _graph is None:
        _graph = EngagementGraph()
    return _graph

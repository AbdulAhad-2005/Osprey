...
class JobStore:
    ...
    def create_and_spawn(self, req: JobStartRequest) -> JobSummary:
        ...
        # Resolve the owning event loop before publishing any in-memory or
        # durable job state.  The old order inserted a QUEUED record first and
        # only then discovered that a synchronous caller had no loop, leaving a
        # phantom job which could never run or reach a terminal state.
        try:

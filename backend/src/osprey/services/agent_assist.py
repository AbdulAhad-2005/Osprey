"""Backward-compatible assist API — implementation lives in platform kernel."""

from __future__ import annotations

from typing import Any

from osprey.platform.adaptation import AdaptationContext, enrich_tool_result
from osprey.platform.run_context import RunAssistState, tool_call_signature
from osprey.schemas.tools import ToolExecutionResponse

# Legacy alias
AssistState = RunAssistState


def enrich_tool_result_for_agent(
    base_text: str,
    tool_response: ToolExecutionResponse,
    *,
    assist_state: RunAssistState,
    signature: str,
    engagement_id: str,
    run_id: str,
    target: str,
    phase: str,
    params: dict[str, Any],
    user_goal: str = "",
) -> str:
    return enrich_tool_result(
        base_text,
        AdaptationContext(
            tool_response=tool_response,
            assist_state=assist_state,
            signature=signature,
            engagement_id=engagement_id,
            run_id=run_id,
            target=target,
            run_phase=phase,
            params=params,
            user_goal=user_goal,
        ),
    )


__all__ = [
    "AssistState",
    "RunAssistState",
    "enrich_tool_result_for_agent",
    "tool_call_signature",
]

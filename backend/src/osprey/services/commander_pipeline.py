...
Runs the deterministic conductor (``investigation_director.
run_agent_driven_pipeline`` — plans/harness/09-dual-mode-planner.md) IN THE
...
async def run_pipeline_foreground(
    ...
    """
    from osprey.services.investigation_director import run_agent_driven_pipeline
    ...
    try:
        summary = await run_agent_driven_pipeline(engagement_id=eid, run_id=rid, on_event=_on_event)

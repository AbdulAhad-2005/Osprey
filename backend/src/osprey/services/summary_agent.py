"""Summary agent: deterministic parse (ground truth) + LLM compression for context.

Two layers, one concern:
- ``summarize_execution`` — parser only. Runs once inside ``tool_execution`` after a
  successful run, writes findings to the store + engagement graph. This is ground truth.
- ``compress_tool_output`` / ``summarize_phase`` — LLM layer. Compresses large stdout
  and writes end-of-phase briefs for the Commander. Never contradicts parsed findings.
"""

from __future__ import annotations

import json
import logging
import re
from typing import Any

from osprey.platform.handoff import export_structured_findings
from osprey.schemas.agent_run import PhaseBrief, ToolSummary
from osprey.schemas.finding import (
    EvidenceGrade,
    Finding,
    FindingConfidence,
    FindingType,
)
from osprey.schemas.tools import ToolExecutionResponse
from osprey.services.engagement_graph import get_engagement_graph
from osprey.services.findings_store import get_findings_store
from osprey.services.llm_service import LLMService, LLMServiceError, get_llm_service
from osprey.services.parsers.registry import ensure_parsers_loaded, parse_tool_output
from osprey.services.skills_loader import load_skills_for_agent

logger = logging.getLogger(__name__)

ensure_parsers_loaded()

COMPRESS_THRESHOLD = 4000
_MAX_STDOUT_FOR_LLM = 12000

_DEFAULT_SUMMARY_SYSTEM = (
    "You compress penetration-test tool output for other agents. "
    "Preserve IPs, hostnames, ports, URLs, and errors. Never invent findings."
)


# === Parser layer (ground truth) ===

async def summarize_execution(
    response: ToolExecutionResponse,
    *,
    engagement_id: str = "",
    run_id: str = "",
    target: str = "",
    phase: str = "",
    force_raw_observation: bool = False,
) -> list[Finding]:
    """Deterministic parse — registry parsers + freeform script/shell extraction.

    Script/shell stdout is mined for URL/status/port/resolve lines so observed
    probe facts become durable findings (not only chat memory).
    """
    findings: list[Finding] = []
    stdout = response.stdout or ""
    stderr = response.stderr or ""
    tool_name = response.tool_name or ""

    if stdout:
        findings = await parse_tool_output(
            tool_name,
            stdout,
            engagement_id=engagement_id,
            run_id=run_id,
            target=target,
            phase=phase,
        )

    needs_freeform = (
        tool_name.startswith("script:")
        or tool_name.startswith("shell:")
        or not findings
        or all(f.finding_type == FindingType.OBSERVATION for f in findings)
    )
    if stdout and needs_freeform:
        from osprey.services.parsers.freeform_probe import parse_freeform_probe_output

        freeform = parse_freeform_probe_output(
            stdout,
            engagement_id=engagement_id,
            run_id=run_id,
            target=target,
            source_tool=tool_name or "script",
        )
        if freeform:
            if findings and all(f.finding_type == FindingType.OBSERVATION for f in findings):
                findings = freeform
            else:
                findings = list(findings) + freeform

    if force_raw_observation and not findings and (stdout or stderr or response.error):
        blob = stdout or stderr or (response.error or "")
        status = "ok" if response.success else "failed"
        findings = [
            Finding(
                engagement_id=engagement_id,
                run_id=run_id,
                phase=phase or "",
                finding_type=FindingType.OBSERVATION,
                title=f"{tool_name} raw output ({status})",
                description=(
                    f"Unparsed/fallback observation from {tool_name} "
                    f"(exit={response.returncode}, success={response.success})"
                ),
                # `evidence` must be evidence OF something — the tool's actual
                # output, not the shell command that produced it (that was the
                # bug: every consumer reading .evidence for "what did this tool
                # find" saw a curl/nmap command line instead of a single byte
                # of real output). The command is still available in metadata
                # for anyone who wants to reproduce the call; raw_data keeps
                # the full untruncated blob as before.
                evidence=blob[:2000],
                confidence=FindingConfidence.HYPOTHESIS,
                evidence_grade=EvidenceGrade.UNVERIFIED,
                source_tool=tool_name,
                target=target,
                tags=["raw_output", "unparsed" if not response.success else "observation"],
                metadata={"command": (response.command or "")[:500]},
                raw_data=blob[:24000],
                notes=stderr[:4000] if stderr else "",
            )
        ]
    elif (
        force_raw_observation
        and findings
        and tool_name.startswith(("script:", "shell:"))
    ):
        structured_n = sum(
            1 for f in findings if f.finding_type != FindingType.OBSERVATION
        )
        if structured_n:
            findings.append(
                Finding(
                    engagement_id=engagement_id,
                    run_id=run_id,
                    phase=phase or "",
                    finding_type=FindingType.OBSERVATION,
                    title=f"{tool_name} run summary ({structured_n} structured)",
                    description="Script/shell produced structured findings via freeform parse",
                    evidence=(response.command or "")[:500],
                    confidence=FindingConfidence.LIKELY,
                    evidence_grade=EvidenceGrade.INFERRED,
                    source_tool=tool_name,
                    target=target,
                    tags=["raw_output", "script_audit"],
                    raw_data=(stdout or "")[:4000],
                )
            )

    if findings:
        get_findings_store().add_many(findings)
        get_engagement_graph().ingest_many(findings)

    # Operator graph links from REL| markers (script/shell) — separate from finding ingest
    if stdout and engagement_id and (
        tool_name.startswith("script:")
        or tool_name.startswith("shell:")
        or "REL|" in stdout
    ):
        try:
            from osprey.services.operator_memory import apply_script_rel_markers

            apply_script_rel_markers(
                stdout,
                engagement_id=engagement_id,
                run_id=run_id,
                seed_target=target,
            )
        except Exception:
            logger.debug("REL marker apply failed", exc_info=True)

    return findings


# === LLM compression layer ===

async def compress_tool_output(
    response: ToolExecutionResponse,
    *,
    finding_titles: list[str],
    llm: LLMService | None = None,
) -> ToolSummary:
    """Compress large stdout for phase-agent context. Parser findings are ground truth."""
    stdout = response.stdout or ""
    if len(stdout) < COMPRESS_THRESHOLD and not finding_titles:
        return _deterministic_tool_summary(response, finding_titles=finding_titles)

    llm = llm or get_llm_service()
    fact_lines = [f"- {title}" for title in finding_titles[:50]]

    user = (
        f"TOOL: {response.tool_name}\n"
        f"SUCCESS: {response.success}\n"
        f"EXIT: {response.returncode}\n"
        f"COMMAND: {response.command or ''}\n\n"
        f"PARSER FINDINGS (ground truth — prefer over stdout interpretation):\n"
        f"{chr(10).join(fact_lines) or '(none)'}\n\n"
        f"STDOUT ({len(stdout)} chars, truncated):\n{stdout[:_MAX_STDOUT_FOR_LLM]}\n\n"
        f"STDERR:\n{(response.stderr or '')[:1500]}\n\n"
        "Respond with JSON only:\n"
        '{"key_facts": ["..."], "errors": ["..."], "prose": "5-10 sentences max"}'
    )

    try:
        result = await llm.complete(
            messages=[
                {"role": "system", "content": load_skills_for_agent("summary") or _DEFAULT_SUMMARY_SYSTEM},
                {"role": "user", "content": user},
            ],
            temperature=0.0,
            max_tokens=1200,
        )
        parsed = _parse_json_object(result["choices"][0]["message"].get("content", "") or "")
        if parsed:
            return ToolSummary(
                tool_name=response.tool_name,
                success=response.success,
                command=response.command or "",
                key_facts=_as_str_list(parsed.get("key_facts")) or list(finding_titles[:50]),
                errors=_as_str_list(parsed.get("errors")),
                prose=str(parsed.get("prose", ""))[:2000],
            )
    except LLMServiceError:
        logger.warning("Summary LLM failed for %s — using deterministic fallback", response.tool_name)

    return _deterministic_tool_summary(response, finding_titles=finding_titles)


async def summarize_phase(
    phase: str,
    *,
    target: str,
    resolved_ip: str | None,
    tool_summaries: list[ToolSummary],
    engagement_id: str = "",
    run_id: str = "",
    llm: LLMService | None = None,
) -> PhaseBrief:
    """Build end-of-phase brief summarizing what this phase produced."""
    exported = export_structured_findings(
        engagement_id=engagement_id,
        run_id=run_id,
        target=target,
        resolved_ip=resolved_ip,
        from_phase=phase,
    )

    brief = PhaseBrief(
        phase=phase,
        target=target,
        resolved_ip=resolved_ip,
        tool_summaries=tool_summaries,
        findings_by_type=exported.findings_by_type,
    )

    if not tool_summaries and not exported.findings_by_type:
        brief.prose_summary = f"No tool runs recorded for {phase}."
        return brief

    llm = llm or get_llm_service()
    tool_digest = "\n".join(
        f"- {ts.tool_name}: {'ok' if ts.success else 'fail'} — {ts.prose[:300]}"
        for ts in tool_summaries[-15:]
    )
    user = (
        f"PHASE: {phase}\nTARGET: {target}\n"
        f"RESOLVED_IP: {resolved_ip or 'unknown'}\n\n"
        f"STRUCTURED FINDINGS:\n{exported.text_summary[:2000]}\n\n"
        f"TOOL SUMMARIES:\n{tool_digest}\n\n"
        "Write a 8-12 sentence phase narrative summarizing what this phase found. "
        "Do not invent facts. Respond with plain text only."
    )

    try:
        result = await llm.complete(
            messages=[
                {"role": "system", "content": load_skills_for_agent("summary") or _DEFAULT_SUMMARY_SYSTEM},
                {"role": "user", "content": user},
            ],
            temperature=0.1,
            max_tokens=900,
        )
        brief.prose_summary = (result["choices"][0]["message"].get("content", "") or "").strip()
    except LLMServiceError:
        logger.warning("Phase summary LLM failed for %s", phase)
        brief.prose_summary = _deterministic_phase_prose(brief)

    return brief


def format_tool_summary_for_context(summary: ToolSummary) -> str:
    """Compact block injected into phase-agent message history."""
    lines = [f"SUMMARY ({summary.tool_name}, {'success' if summary.success else 'failed'}):"]
    if summary.key_facts:
        lines.append("KEY FACTS:")
        lines.extend(f"  - {fact}" for fact in summary.key_facts[:25])
    if summary.errors:
        lines.append("ERRORS:")
        lines.extend(f"  - {err}" for err in summary.errors[:8])
    if summary.prose:
        lines.append(f"NARRATIVE: {summary.prose}")
    return "\n".join(lines)


# === Helpers ===

def _deterministic_tool_summary(
    response: ToolExecutionResponse,
    *,
    finding_titles: list[str],
) -> ToolSummary:
    errors: list[str] = []
    if response.error:
        errors.append(response.error)
    if not response.success and response.stderr:
        errors.append(response.stderr.strip()[:500])

    prose = (
        f"{'Succeeded' if response.success else 'Failed'} "
        f"(exit {response.returncode}). {len(finding_titles)} structured finding(s)."
    )
    preview = (response.stdout or "")[:400]
    if preview:
        prose += f" Output preview: {preview}"

    return ToolSummary(
        tool_name=response.tool_name,
        success=response.success,
        command=response.command or "",
        key_facts=list(finding_titles[:40]),
        errors=errors,
        prose=prose,
    )


def _deterministic_phase_prose(brief: PhaseBrief) -> str:
    parts = [
        f"Phase {brief.phase} on {brief.target or 'unknown target'}.",
        f"Tools run: {len(brief.tool_summaries)}.",
    ]
    if brief.coverage_completed:
        parts.append(f"Completed: {', '.join(brief.coverage_completed)}.")
    if brief.coverage_gaps:
        parts.append(f"Gaps: {', '.join(brief.coverage_gaps)}.")
    total = sum(len(v) for v in brief.findings_by_type.values())
    parts.append(f"Structured findings: {total}.")
    return " ".join(parts)


def _as_str_list(value: Any) -> list[str]:
    if not isinstance(value, list):
        return []
    return [str(item) for item in value if item]


def _parse_json_object(text: str) -> dict[str, Any] | None:
    text = (text or "").strip()
    if not text:
        return None
    try:
        parsed = json.loads(text)
        return parsed if isinstance(parsed, dict) else None
    except json.JSONDecodeError:
        pass
    fenced = re.findall(r"```(?:json)?\s*([\s\S]*?)```", text, flags=re.IGNORECASE)
    for block in fenced:
        try:
            parsed = json.loads(block.strip())
            if isinstance(parsed, dict):
                return parsed
        except json.JSONDecodeError:
            continue
    start = text.find("{")
    end = text.rfind("}")
    if start != -1 and end > start:
        try:
            parsed = json.loads(text[start : end + 1])
            return parsed if isinstance(parsed, dict) else None
        except json.JSONDecodeError:
            return None
    return None
